...
```bash
python main.py -m experiment_name=cw10_cl_hide_lora seed=42 env_params=multi_domain_ft env_params.eval_env_names=cw10_v2 run_params=finetune_coff eval_params=finetune_md_cl agent_params=cdt_lora_mpdt_disc +agent_params.steps_per_task=10000 +agent_params.eval_tii_steps=1000 agent_params/model_kwargs=mdmpdt_mtdmc agent_params/data_paths=cw10_v2_cwnet_2M +agent_params/replay_buffer_kwargs=mtdmc_ft +agent_params.replay_buffer_kwargs.kind=continual agent_params/model_kwargs/prompt_kwargs=hide_lora
```
