...
def setup_wandb(config):
    ...
    if config.use_wandb:
        ...
        run = wandb.init(
            tags=[config.experiment_name], config=config_dict, **config.wandb_params
        )
...
def make_agent(config, env, logdir):
    ...
    agent_params_dict = omegaconf.OmegaConf.to_container(
        config.agent_params, resolve=True, throw_on_missing=True
    )
    agent_kind = agent_params_dict.pop("kind")
    agent_load_path = agent_params_dict.pop("load_path", None)
    agent_load_path = (
        Path(agent_load_path["dir_path"]) / agent_load_path["file_name"]
    ...
    if agent_kind in AGENT_CLASSES.keys():
        ...
        max_state_dim, max_act_dim = (
            config.agent_params.replay_buffer_kwargs.get("max_state_dim", None),
        ...
        dt_config = DecisionTransformerConfig(
            state_dim=state_dim, act_dim=act_dim, **agent_huggingface_params
        )
        ...
        if ou_noise:
            action_noise = (
                OrnsteinUhlenbeckActionNoise(
                    mean=np.zeros(act_dim), sigma=action_noise_std * np.ones(act_dim)
                )
        ...
        agent = get_agent_class(agent_kind)(
            policy, env, **add_agent_kwargs, **agent_params_dict
        )
    elif agent_kind in ["SAC", "ContinualSAC"]:
        policy, policy_kwargs = (
            agent_params_dict.pop("policy"),
            agent_params_dict.pop("policy_kwargs", {}),
        )
        ...
        if extra_encoder:
            ...
            policy_kwargs.update(
                {
                    "features_extractor_class": FlattenExtractorWithMLP,
...
def main(config):
    ...
    try:
        ...
        score = (
            None
            if res is None
            else safe_mean([ep_info["r"] for ep_info in res.ep_info_buffer])
        )
        if ddp:
