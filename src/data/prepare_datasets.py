...
def prepare_datasets(dir_path, save_dir, to_full_obs_space=False, map_names=False):
    ...
    for path in tqdm(paths):
        ...
        write_to_npz(
            obj, save_path, to_full_obs_space, file_name=file_name, map_names=map_names
        )
...
def write_to_npz(obj, save_path, to_full_space=False, map_names=False, file_name=None):
    ...
    if to_full_space:
        ...
        observations = map_flattened_obs_to_full_space(
            observations, env.observation_spec()
        )
        next_observations = map_flattened_obs_to_full_space(
            next_observations, env.observation_spec()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_dir",
        default="/home/chufanchen/Beta/data/metaworld_v2_cwnet_2M",
        type=str,
    )
    parser.add_argument(
        "--save_dir", default="/home/chufanchen/Beta/data/mt50", type=str
    )
    parser.add_argument(
        "--to_full_space",
        action="store_true",
        help="Should only be used with DMControl envs.",
    )
    parser.add_argument(
        "--map_names",
        action="store_true",
