...
class MultiDomainTrajectoryReplayBuffer(TrajectoryReplayBuffer):
    def __init__(
        ...
        **kwargs,
        ...
        It assumes that all trajectories from a particular domain are either in memory or on disk, but no mixture.
    ...
    def compute_trajectory_probs(self, top_k=5, weight_by="len"):
        ...
        if self.trj_ds_has_changed or self.trajectory_probs is None:
            ...
            if weight_by == "uniform":
                ...
                for i, indices in self.domain_to_indices.items():
                    trj_lens = [
                        len(self.trajectories[idx])
                        if isinstance(self.trajectories[idx], Trajectory)
    ...
    def make_sampler(self, dataset, trajectory_probs, batch_size):
        ...
        batch_size = (
            batch_size if not self.ddp else batch_size * int(os.environ["WORLD_SIZE"])
        )

        if self.mixed_weighted:
            # mix batches in proporition to domain
            if (
                self.trj_ds_has_changed or self.domain_weights is None
            ) and not self.fixed_domain_weights:
                ...
                self.domain_weights = {
                    i: total_samples_per_domain[i] / total_samples
                    for i in total_samples_per_domain
                }
            sampler = MixedBatchRandomSampler(
                weights=trajectory_probs,
                domain_weights=self.domain_weights,
                batch_size=batch_size,
                num_samples=len(dataset) * mult,
                replacement=True,
            )
        else:
            sampler = DomainWeightedRandomSampler(
                weights=trajectory_probs,
                batch_size=batch_size,
                num_samples=len(dataset) * mult,
                replacement=True,
