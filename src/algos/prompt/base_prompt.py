...
class Prompt(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "inv_counts_so_far", torch.ones(self.pool_size, requires_grad=False)
        )
    ...
    def aggregate_embeds(
        self, x_embed, cls_features=None, attention_mask=None, tok_to_pos=None
    ):
        if self.agg_token != "all":
            if tok_to_pos is not None:
                ...
                x_embed = x_embed.reshape(
                    batch_size, seq_len // num_tokens, num_tokens, embed_dim
                )
                if self.agg_token == "concat":
                    x_embed = x_embed.reshape(batch_size, seq_len // num_tokens, -1)
                    attention_mask = attention_mask.reshape(
                        batch_size, seq_len // num_tokens, num_tokens
                    )[..., -1].flatten(1)
                else:
                    token_pos = tok_to_pos[self.agg_token]
                    x_embed = x_embed[:, :, token_pos].reshape(
                ...
                token_pos, num_tokens = (
                    self.token_to_pos[self.agg_token],
                    len(self.token_to_pos),
                )
                assert (
                    x_embed.shape[1] % num_tokens == 0
                    and attention_mask.shape[1] % num_tokens == 0
                )
                x_embed = x_embed[:, token_pos::num_tokens]
                attention_mask = attention_mask[:, token_pos::num_tokens]
        if self.embed_key in [
            "mean",
            "last",
            "first",
            "embed",
            ...
            if attention_mask is not None:
                # masked mean
                x_embed_mean = torch.sum(
                    x_embed * attention_mask.float().unsqueeze(-1), dim=1
