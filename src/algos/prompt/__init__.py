...
from .hide_lora import HiDeLoRAPool
...
def make_prompt(config, prompt_kwargs):
    ...
    if prompt_kind == "hopfield":
        prompt = HopfieldPrompt(
            embed_dim=config.hidden_size,
            pattern_size=config.hidden_size,
            **prompt_kwargs,
        )
    ...
    elif prompt_kind == "hide_lora":
        prompt = HiDeLoRAPool(embed_dim=config.hidden_size, **prompt_kwargs)
    else:
