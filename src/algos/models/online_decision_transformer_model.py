...
class OnlineDecisionTransformerModel(DecisionTransformerModel):
    def __init__(
        ...
        reward_condition_only=False,
        stochastic_policy=True,
        gating=False,
        relative_pos_embds=False,
        global_pos_embds=False,
        both_time_embds=False,
        separate_prompt_embed=False,
        learnable_prompt=False,
        use_crossattn_mask=True,
        detach_prompt_embed=False,
        share_prompt_pos_embed=False,
        ...
        self.is_image_space = preprocessing.is_image_space(
            self.observation_space, check_channels=False
        )
        ...
        if self.both_time_embds:
            self.embed_timestep_rel = nn.Embedding(
                config.max_length, config.hidden_size
            )
        ...
        if self.is_image_space:
            self.embed_image = make_image_encoder(
                ...
                features_dim=config.hidden_size,
                encoder_kwargs=self.encoder_kwargs,
            )
        if self.is_discrete:
            self.embed_action = nn.Sequential(
                nn.Embedding(self.action_space.n, config.hidden_size), nn.Tanh()
            )
        if self.separate_ln:
            self.embed_ln = nn.ModuleDict(
                {
                    "s": nn.LayerNorm(config.hidden_size),
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if (
                    ...
                    or isinstance(m, torch.nn.Parameter)
        ...
        if self.stochastic_policy:
            if self.num_task_heads > 1:
                self.mu = nn.ModuleList(
                    [
                        self.make_head(action_pred_in_dim, act_dim, self.n_layer_head)
                ...
                self.log_std = self.make_head(
                    action_pred_in_dim, act_dim, self.n_layer_head
                )
            self.action_dist = (
            ...
            if self.num_task_heads > 1:
                self.action_pred = nn.ModuleList(
                    [
                        nn.Sequential(
                            *(
                                [
    ...
    def setup_prompt(self):
        if self.separate_prompt_embed:
            self.prompt_embed_timestep = nn.Embedding(
            ...
            self.prompt_embed_state = torch.nn.Linear(
                self.config.state_dim, self.config.hidden_size
            )
            self.prompt_embed_action = torch.nn.Linear(
                self.config.act_dim, self.config.hidden_size
            )
            self.prompt_embed_ln = torch.nn.LayerNorm(self.config.hidden_size)
        if self.learnable_prompt:
            prompt_kwargs = (
                {} if self.prompt_kwargs is None else self.prompt_kwargs.copy()
            )
    ...
    def predict_action(self, x, deterministic=False, task_id=None):
        ...
        action = self.action_dist.actions_from_params(
            mean_actions, log_std, deterministic=deterministic
        )
    ...
    def compute_hidden_states(
        ...
        if prompt is not None or (
            self.learnable_prompt and np.random.rand() < self.prompt_prob
        ):
            if self.learnable_prompt:
                ...
                if learnable_prompt_inputs is not None:
                    prompt_stacked_inputs = learnable_prompt_inputs[
                ...
                (
                    prompt_stacked_inputs,
                    prompt_stacked_attention_mask,
                ) = self.compute_prompt_inputs(prompt)
            if prompt_stacked_inputs is None and prompt_stacked_attention_mask is None:
                ...
                prompt_attention_mask = (
                    prompt_stacked_attention_mask if self.use_crossattn_mask else None
                )

        # make position ids
        if self.global_pos_embds:
            position_ids = torch.arange(
                stacked_attention_mask.shape[1],
        ...
        encoder_outputs = self.encoder(
            inputs_embeds=stacked_inputs,  # [batch_size, seq_len x token_len, hidden_size]
            attention_mask=stacked_attention_mask,  # [batch_size, seq_len x token_len]
    ...
    def embed_inputs(self, states, actions, returns_to_go, rewards, attention_mask):
        ...
        action_embeddings = self.get_action_embeddings(
            actions, attention_mask=attention_mask
        )
    ...
    def compute_prompt_inputs(self, prompt):
        ...
        if self.use_time_embds:
            if self.share_prompt_pos_embed:
                time_embeddings = self.get_time_embeddings(
                    timesteps.long(), attention_mask=attention_mask
                ).detach()
    ...
    def compute_learnable_prompt_inputs(
        self,
        stacked_inputs,
        stacked_attention_mask,
        ...
        if prompt_stacked_inputs is not None:
            if self.prompt.prefix:
                ...
                prompt_stacked_attention_mask = torch.ones(
                    shape[1], shape[3], device=prompt_stacked_inputs[0].device
                )
            else:
                if isinstance(prompt_stacked_inputs, (tuple, list)):
                    if isinstance(prompt_stacked_inputs[0], (tuple, list)):
                        ...
                        prompt_stacked_attention_mask = torch.ones(
                            prompt_stacked_inputs[0].shape[:2],
                            device=prompt_stacked_inputs[0].device,
                        )
                else:
                    prompt_stacked_attention_mask = torch.ones(
                        prompt_stacked_inputs.shape[:2],
                        device=prompt_stacked_inputs.device,
                    )
        return {
            "prompt_stacked_inputs": prompt_stacked_inputs,
    ...
    def get_predictions(
        ...
        action_log_probs, reward_preds, action_logits, entropy, return_preds = (
            None,
            None,
            None,
            None,
        ...
        if with_log_probs:
            action_preds, action_log_probs = self.action_log_prob(
                x[:, self.tok_to_pred_pos["a"]], task_id=task_id
            )
        else:
            action_preds = self.predict_action(
                x[:, self.tok_to_pred_pos["a"]],
                deterministic=deterministic,
                task_id=task_id,
            )
        return (
    ...
    def freeze(
        ...
        if exclude_prompt:
            exclude_keys.update(
                [name for name, _ in self.named_parameters() if "prompt" in name]
            )
        if exclude_action_head:
            if self.stochastic_policy:
                exclude_keys.update(
                    [f"mu.{name}" for name, _ in self.mu.named_parameters()]
                )
                exclude_keys.update(
                    [f"log_std.{name}" for name, _ in self.log_std.named_parameters()]
                )
            else:
                if hasattr(self, "action_pred"):
                    exclude_keys.update(
                        [
                            f"action_pred.{name}"
                            for name, _ in self.action_pred.named_parameters()
                        ]
                    )
        ...
        if exclude_adapters:
            exclude_keys.update(
                [name for name, _ in self.named_parameters() if "adapter" in name]
            )
        if exclude_img_encoder:
            exclude_keys.update(
                [name for name, _ in self.named_parameters() if "embed_image" in name]
            )
        if exclude_embeds:
            exclude_keys.update(
                [
        ...
        if exclude_layers is not None:
            assert isinstance(exclude_layers, list) and all(
                [l < 0 for l in exclude_layers]
            )
            layer_idx = list(range(self.config.n_layer))
            exclude_layer_prefix = [
                f"encoder.h.{str(layer_idx[l])}" for l in exclude_layers
            ]
            exclude_keys.update(
                [
                    name
