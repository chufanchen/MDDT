...
class UDT(DecisionTransformerSb3):
    ...
    def _setup_loss_fns(self, reduction="mean"):
        ...
        if self.target_return_loss_fn_type is not None:
            self.target_return_loss_fn = make_loss_fn(
                self.target_return_loss_fn_type
            ).to(self.device)
            self.ddp_kwargs["predict_return"] = True

        if self.target_reward_loss_fn_type is not None:
            assert self.target_reward is not None
            self.target_reward_loss_fn = make_loss_fn(
                self.target_reward_loss_fn_type
            ).to(self.device)
    ...
    def _setup_policy(self):
        ...
        if self.detach_policy_dt:
            ...
            self.optimizer = make_optimizer(
                self.optimizer_kind,
                self.policy.get_action_head_params(),
    ...
    def _setup_critic(self):
        if self.use_critic:
            ...
            self.critic = critic_class(
                ...
                share_features_extractor=self.critic_share_extractor
                if not self.update_with_critic
                else False,
                **critic_kwargs,
            )
            ...
            for param in self.critic_target.parameters():
                param.requires_grad = False
            if (
                self.critic_as_dt_head
                and self.critic_share_extractor
                and self.use_policy_target
            ):
            ...
            if self.critic_as_dt_head:
                if self.warmup_steps > 0 and (
                    not self.critic_share_extractor or self.update_with_critic
                ):
                    ...
                    self.critic_schedulers["critic_warmup"] = (
    ...
    def update_policy(
        ...
        if (
            self._n_updates % self.target_update_interval == 0
            and self.target_step_kind == "update"
        ):
            if self.use_critic:
    ...
    def compute_main_policy_loss(
            ...
            if self.critic_as_dt_head:
                ...
                if self.critic_share_extractor:
                    if self.use_dummy_udt:
                        q_input = policy_output.last_encoder_output.reshape(
                            -1, self.policy.config.hidden_size
                        )[attention_mask.reshape(-1) > 0]
                    else:
                        q_input = policy_output.last_encoder_output[
                            :, self.dt_state_context_loc
                        ].reshape(-1, self.policy.config.hidden_size)[
                            attention_mask.reshape(-1) > 0
                        ]
                else:
                    q_input = self.critic.extract_features(
                        ...
                        timesteps=timesteps.long(),
                        attention_mask=attention_mask,
                        raw=self.use_dummy_udt,
                    )
                    q_input = q_input.reshape(-1, self.policy.config.hidden_size)[
                        attention_mask.reshape(-1) > 0
                    ]
                ...
                masked_states = state_targets.reshape(-1, state_targets.shape[-1])[
                    attention_mask.reshape(-1) > 0
                ]
                ...
                q_input = state_targets.reshape(-1, state_targets.shape[-1])[
                    attention_mask.reshape(-1) > 0
                ]
        ...
        if self.loss_fn_type == "td3":
            ...
            with torch.no_grad():
                ...
                target_q_values = (reward * self.critic_reward_scale) + (
                    1 - dones
                ) * self.critic_gamma * next_q_values

            # masking
            mask = attention_mask[:, :-1].reshape(-1) > 0
            target_q_values = target_q_values.reshape(-1, 1)[mask]
            current_q_values = policy_output.action_preds[:, :-1].reshape(-1, act_dim)[
                mask
            ]
            # get q-values for actions from the replay buffer
            action_targets = action_targets[:, :-1].reshape(
                -1, action_targets.shape[-1]
            )[mask]
            current_q_values = torch.gather(
                current_q_values, dim=1, index=action_targets.long()
            )
            ...
            if self.loss_fn_type == "ce" or self.loss_fn_type == "dist_ce":
                ...
                action_target_tokens = (
                    self.policy.tokenize_actions(action_targets)
                    if self.policy.tokenize_a and is_continuous_action
                ...
                if is_continuous_action:
                    ...
                    action_preds = (
                        policy_output.action_preds
                        if not self.last_seq_only
                        else policy_output.action_preds[:, -1]
                    )
                    action_preds = action_preds.reshape(-1, act_dim)[
                        attention_mask.reshape(-1) > 0
                    ]
    ...
    def update_critic(
        ...
        if self.policy_target_as_dt_head or self.critic_as_dt_head:
            if self.critic_share_extractor and not self.update_with_critic:
                last_encoder_output = (
                    policy_output.last_encoder_output.detach()
                    if self.use_dummy_udt
                    else policy_output.last_encoder_output[
                        :, self.dt_state_context_loc
                    ].detach()
                )
            else:
        ...
        with torch.no_grad():
            ...
            if self.use_policy_target:
                ...
                next_actions = torch.cat(
                    [
                        action_targets,
                        torch.zeros_like(action_targets[:, -1]).unsqueeze(1),
                    ],
                    dim=1,
                )
                next_rewards = torch.cat(
                    [rewards, torch.zeros_like(rewards[:, -1]).unsqueeze(1)], dim=1
                )
                next_returns_to_go = torch.cat(
                    [
                        rewards_to_go,
                        (rewards_to_go[:, -1] - rewards[:, -1]).unsqueeze(1),
                    ],
                    dim=1,
                )
                # ensure timesteps is is not OOB
                next_ts = (timesteps[:, -1] + 1).unsqueeze(1)
                next_ts[next_ts >= self.policy.config.max_ep_len] = (
                    self.policy.config.max_ep_len - 1
                )
                next_timesteps = torch.cat([timesteps, next_ts], dim=1)
                next_attention_mask = torch.cat(
                    [
                        attention_mask,
                        torch.ones_like(attention_mask[:, -1]).unsqueeze(1),
                    ],
                    dim=1,
                )
                # shift
                (
                    ...
                    next_actions[:, 1:],
                    next_rewards[:, 1:],
                    next_returns_to_go[:, 1:],
                    next_timesteps[:, 1:],
                    next_attention_mask[:, 1:],
                )
            ...
            if self.target_policy_noise > 0:
                noise = (
                    next_action.clone()
                    .data.normal_(0, self.target_policy_noise)
                    .clamp(-0.5, 0.5)
                )
            ...
            if self.critic_as_dt_head:
                if self.critic_share_extractor and self.use_policy_target:
                    ...
                    next_q_values = self.critic_target(
                        features=next_features, action_preds=next_action, obs=next_state
                    )
                else:
        ...
        if self._n_updates % 100 == 0 and self.debug:
            self.grad_plotter.plot_grad_flow(
                ...
                f"critic_update,critic,step={self._n_updates}.png",
            )
            self.grad_plotter.plot_grad_flow(
                self.policy.named_parameters(),
                f"critic_update,policy,step={self._n_updates}.png",
            )
    ...
    def update_critic_target(self):
        ...
        if self.critic_share_extractor and not self.use_policy_target:
            ...
            polyak_update(
                ...
                self.critic_target.features_extractor.parameters(),
                self.tau,
            )

    def update_policy_target(self):
        policy_params = (
            self.policy.action_pred.parameters()
            if self.policy_target_as_dt_head
            else self.policy.parameters()
        )
        polyak_update(policy_params, self.policy_target.parameters(), self.tau)

    def compute_return_prediction_loss(
        self, return_targets, return_preds, attention_mask
    ):
    ...
    def compute_reward_prediction_loss(
        self, reward_targets, reward_preds, attention_mask
    ):
        reward_targets = reward_targets.reshape(-1)[attention_mask.reshape(-1) > 0]
        if self.target_return_loss_fn_type == "ce":
            reward_preds = reward_preds.reshape(-1, reward_preds.shape[-1])[
                attention_mask.reshape(-1) > 0
            ]
    ...
    def compute_action_prediction_loss(
        self, action_targets, action_preds, attention_mask
    ):
    ...
    def compute_state_prediction_loss(self, state_targets, state_preds, attention_mask):
        ...
        state_targets = state_targets.reshape(-1, obs_dim)[
            attention_mask.reshape(-1) > 0
        ]
    ...
    def _on_step(self) -> None:
        ...
        super()._on_step()
        if (
            self.target_step_kind == "env"
            and self.num_timesteps % self.target_update_interval == 0
        ):
            self.update_policy_target()
