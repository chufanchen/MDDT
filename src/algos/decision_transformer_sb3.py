...
from .agent_utils import (
    ...
    make_loss_fn,
    CustomDDP,
    make_random_proj_matrix,
)
from ..buffers import (
    TrajectoryReplayBuffer,
    PromptBuffer,
    ContinualTrajectoryReplayBuffer,
    MultiDomainTrajectoryReplayBuffer,
)
...
class DecisionTransformerSb3(OffPolicyAlgorithm):
    def __init__(
        ...
        self.prompt_buffer_kwargs = (
            prompt_buffer_kwargs if prompt_buffer_kwargs is not None else {}
        )
        self.prompt_buffer_sync_freq = (
            10000 if prompt_buffer_sync_freq is None else prompt_buffer_sync_freq
        )
        self.eval_prompt_update_freq = (
            10000 if eval_prompt_update_freq is None else eval_prompt_update_freq
        )
        ...
        self.ddp_kwargs = {
            "predict_reward": not ddp,
            "predict_return": not ddp,
            "predict_state": not ddp,
        }
    ...
    def _setup_prompt_buffer(self):
        ...
        self.prompt_buffer = PromptBuffer(
            self.buffer_size, self.observation_space, self.action_space, **buffer_kwargs
        )
    ...
    def _setup_policy(self):
        ...
        if self.ddp:
            ...
            self.policy = CustomDDP(
                self.policy, device_ids=[self.device], find_unused_parameters=True
            )
        ...
        if self.lr_sched_kwargs is not None:
            ...
            lr_sched = make_lr_scheduler(
                self.optimizer, kind=lr_sched_kind, sched_kwargs=self.lr_sched_kwargs
            )
            schedulers.append(lr_sched)
            milestones = (
                [self.warmup_steps] if self.warmup_steps > 0 and not load_optim else []
            )
        if len(schedulers) > 0:
            self.schedulers = torch.optim.lr_scheduler.SequentialLR(
                self.optimizer, schedulers, milestones=milestones
            )
        else:
    ...
    def _setup_entropy_tuning(self):
        ...
        if isinstance(self.ent_coef, str) and self.ent_coef.startswith("auto"):
            ...
            self.ent_coef_optimizer = make_optimizer(
                self.optimizer_kind, [self.log_ent_coef], lr=self.lr_entropy
            )
    ...
    def _setup_loss_fns(self, reduction="mean"):
        loss_fn = make_loss_fn(
            self.loss_fn_type, reduction=reduction, label_smoothing=self.label_smoothing
        )
    ...
    def collect_rollouts(
        ...
        target_return_val = self.compute_target_return_val(
            self.env, task_id=self.current_task_id
        )
        target_return_val_tensor = torch.as_tensor(
            target_return_val, device=self.device, dtype=torch.float32
        )
        if self.rollout_buffer == {}:
            ...
            actions = torch.zeros(
                (0, self.action_dim), device=self.device, dtype=torch.float32
            )
            rewards = torch.zeros(0, device=self.device, dtype=torch.float32)
            timesteps = torch.as_tensor(
                0, device=self.device, dtype=torch.long
            ).reshape(1, 1)
            if self.target_reward is not None and (
                self.policy.reward_condition or self.policy.reward_condition_only
            ):
                target_reward = torch.as_tensor(
                    self.target_reward / self.reward_scale,
                    device=self.device,
                    dtype=torch.float32,
                ).reshape(1, 1)
            else:
        ...
        while should_collect_more_steps(
            ...
            actions = torch.cat(
                [actions, torch.zeros((1, self.action_dim), device=self.device)], dim=0
            )
            rewards = torch.cat([rewards, torch.zeros(1, device=self.device)])
            action, buffer_action, action_tensor = self._sample_action(
                states,
                actions,
                target_reward if target_reward is not None else rewards,
                target_return,
                timesteps,
                learning_starts,
                action_noise,
            ...
            actions[-1] = (
                ...
                else torch.from_numpy(action).to(actions.device)
            )
            ...
            self._store_transition(
                replay_buffer, buffer_action, state, buffer_reward, done, info
            )
            ...
            pred_return = torch.minimum(
                target_return[0, -1] - (buffer_reward / self.reward_scale),
                target_return_val_tensor,
            )
            ...
            if target_reward is not None:
                ...
                target_reward = torch.cat(
                    [
                        target_reward,
                        torch.as_tensor(
                            self.target_reward / self.reward_scale, device=self.device
                        ).reshape(1, 1),
                    ],
                    dim=1,
                )
    ...
    def compute_target_return_val(self, env=None, task_id=0):
        if len(self.replay_buffer) > 0 and self.target_return_type == "max":
            if task_id == self.current_task_id:
                target = (
                    self.replay_buffer._get_max_return() * self.target_return_mult
                ) / self.reward_scale
                self.target_return_dict[task_id] = max(
                    self.target_return_dict[task_id], target
                )
            else:
                # in case task_id already in return_dict, use it otherwise target_return=1
                target = (
                    self.target_return_dict[task_id]
                    if task_id in self.target_return_dict
                    else 1
                )
            return target
            ...
            return (
                ALL_TARGETS[env_name] / self.get_reward_scale_for_env(env_name)
            ) * self.target_return_mult
    ...
    def _sample_action(
        ...
        if (
            ...
            if self.use_prompt_buffer:
                if (
                    self.num_timesteps % self.eval_prompt_update_freq == 0
                    and self.rollout_eval_prompt
                ):
    ...
    def pad_inputs(
        self, states, actions, returns_to_go, timesteps, context_len=5, rewards=None
    ):
        padding = context_len - actions.shape[1]
        attention_mask = torch.cat(
            [
                torch.zeros(padding, device=self.device, dtype=torch.long),
        ...
        if self.replay_buffer.max_state_dim is not None and len(states.shape) == 3:
            ...
            states = torch.cat(
                [states, torch.zeros((*states.shape[:-1], s_pad), device=self.device)],
                dim=-1,
            )
        ...
        if self.replay_buffer.max_act_dim is not None and actions.is_floating_point():
            ...
            if len(states.shape) != 5:
                ...
                actions = torch.cat(
                    [
                        actions,
                        torch.zeros((*actions.shape[:-1], a_pad), device=self.device),
                    ],
                    dim=-1,
                )
        ...
        states = torch.cat(
            [
                torch.zeros((states.shape[0], padding, *obs_shape), device=self.device),
                states,
            ],
            dim=1,
        ).float()
        actions = torch.cat(
            [
                torch.zeros((actions.shape[0], padding, act_dim), device=self.device),
                actions,
            ],
            dim=1,
        )
        returns_to_go = torch.cat(
            [
                torch.zeros((returns_to_go.shape[0], padding, 1), device=self.device),
                returns_to_go,
            ],
            dim=1,
        ).float()
        timesteps = torch.cat(
            [
                torch.zeros(
                    (timesteps.shape[0], padding), dtype=torch.long, device=self.device
                ),
                timesteps,
            ],
            dim=1,
        ...
        if rewards is not None:
            rewards = torch.cat(
                [
                    torch.zeros((rewards.shape[0], padding, 1), device=self.device),
                    rewards,
                ],
                dim=1,
            )
    ...
    def get_action_pred(
        ...
        if not is_eval and self.num_timesteps % 10000 == 0 and self.log_attn_maps:
            ...
            if policy_output.cross_attentions is not None:
                self._record_attention_maps(
                    policy_output.cross_attentions,
                    step=self.num_timesteps,
                    prefix="rollout_cross",
                    lower_triu=False,
                )
    ...
    def train(self, gradient_steps: int, batch_size: int) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            if self.stochastic_policy:
                ...
                action_log_probs_masked = action_log_probs.reshape(-1, 1)[
                    attention_mask.reshape(-1) > 0
                ]
                metrics["action_log_probs_mean"].append(
                    action_log_probs_masked.mean().item()
                )
                if policy_output.entropy is not None:
                    entropy_masked = policy_output.entropy.reshape(-1, 1)[
                        attention_mask.reshape(-1) > 0
                    ]
                if self.last_seq_only:
                    ...
                    is_last_seq[:, -1] = 1
                    is_last_seq = (
                        is_last_seq.reshape(-1)[attention_mask.reshape(-1) > 0] > 0
                    )
                    action_log_probs_masked = action_log_probs_masked[is_last_seq]
                    entropy_masked = (
                        entropy_masked[is_last_seq]
                        if entropy_masked is not None
                        else None
                    )

            # update ent_coef
            if self.stochastic_policy and self._n_updates > self.ent_tuning_start:
                ent_coef, ent_coef_dict = self.update_entropy_coef(
                    action_log_probs_masked, entropy=entropy_masked
                )
            ...
            if (self._n_updates + 1) % 10000 == 0 and self.log_attn_maps:
                self._record_attention_maps(
                    policy_output.attentions, step=self.num_timesteps, prefix="train"
                )
                if policy_output.cross_attentions is not None:
                    self._record_attention_maps(
                        policy_output.cross_attentions,
    ...
    def sample_batch(self, batch_size, use_temp=False, use_eval=False):
        ...
        if self.use_prompt_buffer:
            if (
                not self.prompt_buffer.is_empty(self.current_task_id)
                or self.prompt_buffer.multitask_batch
            ):
                prompt = self.prompt_buffer.sample(
                    batch_size=self.batch_size,
                    env=self._vec_normalize_env,
                    task_id=self.current_task_id
                    if not self.prompt_buffer.multitask_batch
                    else task_ids,
                    trj_id=trj_ids,
                )
                prompt = prompt._replace(
    ...
    def update_policy(
        ...
        if self._n_updates % 100 == 0 and self.debug:
            if hasattr(self, "critic"):
                self.grad_plotter.plot_grad_flow(
                    self.critic.named_parameters(),
                    f"policy_update,critic,step={self._n_updates}.png",
                )
            self.grad_plotter.plot_grad_flow(
                self.policy.named_parameters(),
                f"policy_update,policy,step={self._n_updates}.png",
            )
        ...
        if self._n_updates % self.accumulation_steps == 0:
            ...
            if self.debug:
                params_before = {
                    k: v.clone() for k, v in self.policy.named_parameters()
                }
            ...
            if self.debug:
                ...
                has_changed = {
                    k: not torch.equal(params_before[k], params_after[k])
    ...
    def compute_policy_loss(
        ...
        action_preds, action_log_probs = (
            policy_output.action_preds,
            policy_output.action_log_probs,
        )
        # compute loss + update
        loss_dict = {}
        ...
        if self.stochastic_policy:
            if self.loss_fn_type == "nll":
                ...
                action_targets_log_prob = self.policy.compute_log_prob_given_action(
                    action_targets
                )
                loss_actions = -action_targets_log_prob.reshape(-1, 1)[
                    attention_mask.reshape(-1) > 0
                ].mean()
            else:
                # shape: [batch_size, context_len, action_dim] (before masking)
    ...
    def compute_prompt_loss(self, loss, policy_output):
        ...
        if "reduce_sim" in prompt_infos:
            loss = loss - (
                self.pull_constraint_coef * policy_output.prompt_infos["reduce_sim"]
            )
    ...
    def update_entropy_coef(self, action_log_probs, entropy=None):
        ...
        if self.ent_coef_optimizer is not None and self.stochastic_policy:
            ...
            if entropy is not None:
                ...
                ent_coef_loss = -(
                    self.log_ent_coef
                    * (action_log_probs + self.target_entropy).detach()
                ).mean()
    ...
    def learn(
        ...
        buffer_reinit = True
        continue_training = (
            self.load_kwargs.get("load_optim", False)
        ...
        while self.num_timesteps < total_timesteps:
            ...
            if (
                self.reinit_policy_after is not None
                and self.num_timesteps >= self.reinit_policy_after
            ):
    ...
    def _dump_logs(self) -> None:
        ...
        sps = int(
            (samples_seen - samples_seen_at_start)
            / ((time.time() - self.start_time) + 1e-8)
        )
        self.logger.record("time/sps", sps)
        self.logger.record("train/samples_seen", samples_seen)
        if self.ddp:
            self.logger.record(
                "time/sps_per_device", int(sps / int(os.environ["WORLD_SIZE"]))
            )
        super()._dump_logs()
    ...
    def _record_metrics_from_dict(self, metrics, prefix, aggregation=np.mean):
        for k, v in metrics.items():
            if prefix == "train" and (
                ...
                or isinstance(self.replay_buffer, MultiDomainTrajectoryReplayBuffer)
            ):
    ...
    def _on_step(self):
        ...
        if (
            self.use_prompt_buffer
            and self.num_timesteps % self.prompt_buffer_sync_freq == 0
        ):
    ...
    def _sync_prompt_buffer(self):
        trjs = self.replay_buffer._get_topk_trajectories(
            self.prompt_buffer.num_trjs_per_task
        )
    ...
    def _update_eval_prompt(self):
        if not self.prompt_buffer.is_empty(self.current_task_id):
            ...
            self.eval_prompt = self.prompt_buffer.sample(
                batch_size=self.env.num_envs,
                env=self._vec_normalize_env,
                task_id=self.current_task_id,
            )
            self.eval_prompt = self.eval_prompt._replace(
                rewards_to_go=self.eval_prompt.rewards_to_go / self.reward_scale,
    ...
    def _on_task_switch(self):
        ...
        print(
            f"Switching task id to {self.current_task_id} at step {self.num_timesteps}"
        )
        self.current_task_id_tensor = torch.tensor(
            self.current_task_id, device=self.device, dtype=torch.int32
        )
    ...
    def _record_param_count(self):
        ...
        if self.policy.num_task_heads > 1:
            if self.stochastic_policy:
                ...
                logstd_counts = get_param_count(
                    self.policy.log_std, "task_head_log_std"
                )
                counts = {**counts, **mu_counts, **logstd_counts}
            else:
                head_counts = get_param_count(
                    self.policy.action_pred, "task_head_action_pred"
                )
    ...
    def _record_prompt_infos(self, prompt_infos, prefix="prompt"):
        ...
        if "prompt_key" in prompt_infos:
            ...
            prompt_dict[f"selected_key_mean"] = (
                prompt_infos["selected_key"].mean().item()
            )
        ...
        if (
            "prompt_idx" in prompt_infos
            and self._n_updates % 10000 == 0
            and not self.debug
        ):
            if hasattr(self.policy.prompt, "counts"):
                ...
                self.logger.record(
                    f"{prefix}/select_ratio", Figure(fig2, True), exclude="stdout"
                )
        if self._n_updates % 20000 == 0 and not self.debug:
            if "similarity" in prompt_infos:
                ...
                self.logger.record(
                    f"{prefix}/similarity", Figure(fig, True), exclude="stdout"
                )
        return prompt_dict

    def _record_attention_maps(
        self,
        attention_scores,
        prefix="train",
        step=None,
        ...
        if step is None:
            step = self.num_timesteps
        figures = make_attention_maps(
            attention_scores, step, lower_triu=lower_triu, vmin=vmin, vmax=vmax
        )
        for k, v in figures.items():
            self.logger.record(
                f"{prefix}_attn_scores/{k}", Figure(v, True), exclude="stdout"
            )
